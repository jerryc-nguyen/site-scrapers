class ItemParsers::Tiki
  ...
  def detail
    ...
    if page.present?
      ...
      # image_urls_ref += page.search(".magiczoom [data-zoom-image]").map do |item|
      #   item.attr("data-zoom-image").to_s
      # end

      # image_urls_ref = image_urls_ref.compact.select(&:present?)

      # image_book = page.search("#product-magiczoom").attr("data-zoom-image").to_s rescue nil

      # if image_book.present?
      #   image_urls_ref.push(image_book)
      # end

      # image_urls_ref = image_urls_ref.uniq
